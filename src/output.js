...
function printAnalysis(analysis, decision, strategy, sessionPath, modelId) {
  ...
  console.log(C.dim + '  Metrikler:' + C.reset);
  console.log(`    ${C.gray}Mesaj         :${C.reset} ${metrics.messageCount}`);
  console.log(`    ${C.gray}Tool kullanımı:${C.reset} ${metrics.toolUses}`);
  console.log(`    ${C.gray}Dosya değişim :${C.reset} ${metrics.filesModified}`);
  console.log(`    ${C.gray}Output toplamı:${C.reset} ${fmtK(metrics.outputTokens)} token`);
  if (metrics.avgGrowthPerTurn > 0) {
    console.log(`    ${C.gray}Büyüme        :${C.reset} ~${fmtK(metrics.avgGrowthPerTurn)}/tur`);
  }
  console.log('');

  if (reasons.length) {
    console.log(C.dim + '  Analiz:' + C.reset);
    for (const r of reasons) console.log(`    • ${r}`);
    console.log('');
  }

  if (strategy?.keep?.length) {
    console.log(C.green + '  ✓ Özette tut:' + C.reset);
    for (const s of strategy.keep.slice(0, 5)) console.log(`    • ${s}`);
    console.log('');
  }

  if (analysis.lastNMessages?.length) {
    console.log(C.dim + '  Son kullanıcı mesajları:' + C.reset);
...
function printCompactResult(analysis, decision, strategy, sessionPath, modelId, clipboardOk) {
  ...
  console.log(line);
  console.log(C.bold + C.cyan + '  /compact prompt (kopyala-yapıştır):' + C.reset);
  console.log('');
  console.log(C.bold + '  ' + strategy.compactPrompt + C.reset);
  console.log('');
  console.log(line);
  if (clipboardOk) {
    console.log(C.gray + '  ✓ Clipboard\'a kopyalandı' + C.reset);
  }
  console.log('');
}
...
function printSnapshotResult(outPath, indexUpdated) {
  console.log('');
  console.log(C.green + '  ✓ Snapshot yazıldı' + C.reset);
  console.log(`    ${outPath}`);
  if (indexUpdated) console.log(C.gray + `    MEMORY.md index güncellendi` + C.reset);
  console.log('');
}

function printHistory(rows) {
  console.log('');
  console.log(C.bold + `  Son ${rows.length} session:` + C.reset);
  console.log('');
  for (const r of rows) {
