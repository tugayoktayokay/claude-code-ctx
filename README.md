...
> **If you already installed via npm → still works**, but consider migrating (`ctx uninstall-hooks` then `/plugin install ctx@claude-code-ctx`).
...
> 📦 **npm package:** `claude-code-ctx` · **plugin name:** `ctx` · **marketplace:** `claude-code-ctx` → install `ctx@claude-code-ctx`
...
Claude Code reads `.claude-plugin/plugin.json`, registers 6 hooks + MCP server + 29 slash commands automatically. Manage with `/plugin list`, `/plugin disable ctx`, `/plugin update ctx`.

Names: the **plugin** is `ctx` (drives the runtime namespace and MCP prefix), hosted in the **marketplace** `claude-code-ctx`, distributed on **npm** as `claude-code-ctx`. So you install with `ctx@claude-code-ctx` but the commands appear as `/ctx:doctor`, `/ctx:ask`, `/ctx:version` — not `/claude-code-ctx:ctx-doctor`.
...
### Migrating to 0.8.22 (plugin renamed `claude-code-ctx` → `ctx`)

Breaking for plugin installs only. v0.8.22 shortens the plugin name back to `ctx`, so the runtime namespace becomes `/ctx:*` and MCP tools become `mcp__plugin_ctx_ctx__*`. The old `claude-code-ctx` install won't auto-update to the new name — reinstall once:

```
/plugin uninstall claude-code-ctx@claude-code-ctx
/plugin install ctx@claude-code-ctx
```

The npm package and the marketplace handle stay `claude-code-ctx`; only the plugin name changed. `ctx metrics` reads both the legacy `mcp__plugin_claude-code-ctx_*` and new `mcp__plugin_ctx_*` tool names, so historical metrics survive the rename.
